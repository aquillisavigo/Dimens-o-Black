        ...
        <div className="space-y-12 animate-in slide-in-from-bottom-8 duration-700 pb-20">
            ...
            {!pixData ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {plans.map(p => (
                        <div key={p.name} className={`bg-surface-dark border-2 ${p.color} p-10 rounded-[2.5rem] flex flex-col text-center shadow-2xl relative overflow-hidden group transition-all hover:translate-y-[-8px]`}>
                            <div className={`absolute top-0 right-0 w-32 h-32 bg-white/5 rounded-full -mr-16 -mt-16 transition-transform group-hover:scale-110`}></div>

                            <h3 className="text-sm font-bold text-gray-400 uppercase tracking-[0.3em] mb-6 group-hover:text-white transition-colors">
                                {p.name}
                            </h3>

                            <div className="mb-2">
                                <span className="text-2xl font-display font-bold text-white/50 align-top mr-1">R$</span>
                                <span className="text-7xl font-display font-bold text-white tracking-tighter">{p.price}</span>
                            </div>
                            ...
                            <div className="mt-auto">
                                ...
                                <button
                                    ...
                                    className="w-full bg-white text-black font-bold py-5 rounded-2xl uppercase text-xs tracking-widest shadow-lg hover:bg-primary hover:text-white transition-all flex items-center justify-center gap-2 group-hover:shadow-glow"
                                >
                                    {loading && selectedPlan?.id === p.id ? (
                                        <div className="w-4 h-4 border-2 border-black group-hover:border-white border-t-transparent rounded-full animate-spin"></div>
                                    ) : 'Assinar Agora'}
                                </button>
                            </div>
                        </div>
                ...
                <div className="max-w-md mx-auto animate-in zoom-in-95 duration-500">
                    <div className={`bg-surface-dark border-2 ${selectedPlan.color} p-10 rounded-[3rem] text-center shadow-2xl relative overflow-hidden`}>
                        <h3 className="text-white font-bold uppercase text-xs tracking-widest mb-8 text-glow">
                        ...
                        <div className="bg-white p-6 rounded-3xl shadow-2xl mb-8 flex flex-col items-center">
                            ...
                            />
                            <p className="text-[10px] text-gray-400 font-bold mt-4 uppercase tracking-[0.2em] italic">Aguardando Pagamento...</p>
                        </div>

                        <div className="bg-black/40 border border-white/10 rounded-2xl p-5 mb-8 relative">
                            <p className="text-[8px] text-gray-500 font-bold uppercase mb-2 tracking-[0.3em]">Pix Copia e Cola</p>
                            <p className="text-[10px] text-gray-300 truncate pr-12 font-mono">{pixData.pix.code}</p>
                            <button
