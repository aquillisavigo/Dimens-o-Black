...
const OfertasClonadasView: React.FC<OfertasClonadasViewProps> = ({ balance, onPurchase }) => {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700 pb-20">
            <div className="bg-gradient-to-r from-red-950/40 to-black p-12 rounded-[2.5rem] border border-red-500/20 shadow-2xl relative overflow-hidden text-center md:text-left">
            ...
            {loading ? (
                ...
                <div className="text-center py-32 bg-white/5 border border-white/5 rounded-[3rem] border-dashed">
                    <span className="material-symbols-outlined text-6xl text-gray-700 mb-6 block">shopping_basket</span>
                    <p className="text-gray-500 font-bold uppercase tracking-[0.2em] text-xs">Nenhuma oferta disponível no momento.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {products.map(p => (
                        <div key={p.id} className="bg-surface-dark border border-white/5 rounded-[2.5rem] overflow-hidden flex flex-col group hover:border-primary/40 transition-all duration-500 shadow-xl">
                            <div className="h-48 relative bg-black overflow-hidden">
                                <img src={p.image_url || 'https://via.placeholder.com/400x200'} alt={p.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 opacity-50 group-hover:opacity-80" />
                            ...
                            <div className="p-8 flex flex-col flex-1">
                                <div className="mb-6">
                                    <h3 className="text-xl font-display font-bold text-white uppercase tracking-tight mb-2 group-hover:text-primary transition-colors">{p.name}</h3>
