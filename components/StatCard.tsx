...
const StatCard: React.FC<StatData & { onClick?: () => void }> = ({ title, value, change, subtext, icon, color, isPositive, onClick }) => {
  ...
  return (
    <div
      onClick={onClick}
      className={`bg-white dark:bg-surface-dark border border-border-light dark:border-border-dark rounded-xl p-6 shadow-sm hover:border-primary/50 hover:scale-[1.02] hover:shadow-glow transition-all duration-300 group relative overflow-hidden ${onClick ? 'cursor-pointer' : 'cursor-default'}`}
    >
